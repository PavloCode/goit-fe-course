...
const getAllNames = arr => arr.map(name => name.name);
...
console.log('test', getInactiveUsers(users)); // [объект Moore Hensley, объект Ross Vazquez, объект Blackburn Dotson]
...
const getUsersByFriend = (arr, name) => {
    const usersHadValue = arr.filter(user => user.friends.includes(name));
    const usersValue = usersHadValue.map(user => user.name);
    return usersValue;
};
